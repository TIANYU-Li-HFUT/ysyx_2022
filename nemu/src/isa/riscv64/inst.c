...
#define R(i) gpr(i)
#define Mr vaddr_read   // 8 64bit 4 32bit ....
...
#define immU() do { *imm = SEXT(BITS(i, 31, 12), 20) << 12; } while(0)   // shift left 
...
static int decode_exec(Decode *s) {
  ...
  int32_t sign_src1_32;
  s->dnpc = s->snpc;

  sword_t sign_src1,sign_src2;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> BITS(imm,5,0); ); //logic shift right
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = SEXT(BITS(imm + src1,31,0),32) ); 
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, sign_src1_32 = (int32_t) BITS(src1,31,0); R(dest) = SEXT(sign_src1_32 >> BITS(imm,4,0),32) ); 
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2 ); 
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = src1 + src2 ); 
  INSTPAT("??????? ????? ????? 011 ????? 01100 11", sltu   , R, if(src1<src2) R(dest) = 1; else R(dest)=0;); //snez
  INSTPAT("??????? ????? ????? 010 ????? 01100 11", slt    , R, sign_src1=(sword_t)src1; sign_src2=(sword_t)src2; if(sign_src1<sign_src2) R(dest) = 1; else R(dest)=0;); 
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, sign_src1_32 = (int32_t) BITS(src1,31,0); R(dest) = SEXT(sign_src1_32 >> BITS(src2,4,0),32); );   
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, if(src1<imm) R(dest) = 1; else R(dest)=0;); //seqz

  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, sign_src1=(sword_t)src1; R(dest) = sign_src1 >> (BITS(imm,5,0)););  //sign number arithmetic shift right
...
//branch
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, sign_src1=(sword_t)src1; sign_src2=(sword_t)src2;if(sign_src1>=sign_src2) {s->dnpc = imm + s->pc;});  
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, sign_src1=(sword_t)src1; sign_src2=(sword_t)src2;if(sign_src1<sign_src2) {s->dnpc = imm + s->pc;}); 
//jump  
